...
class DecodeVectorCliTests(unittest.TestCase):
    ...
    def test_decodes_rfc8251_testvector01_first_five_packets(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            input_path = root / "testvector01-first-five.bit"
            output_path = root / "out.s16le"
            input_path.write_bytes(first_packet_records(ROOT / "tests/vectors/rfc8251/testvector01.bit", 5))
            ...
            self.assertEqual(completed.returncode, 0, completed.stderr)
            self.assertEqual(len(output_path.read_bytes()), 5440)

    def test_decodes_rfc8251_testvector01_first_thirteen_packets(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            input_path = root / "testvector01-first-thirteen.bit"
            output_path = root / "out.s16le"
            input_path.write_bytes(first_packet_records(ROOT / "tests/vectors/rfc8251/testvector01.bit", 13))
    ...
    def test_decodes_rfc8251_testvector01_first_thirty_four_packets(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            input_path = root / "testvector01-first-thirty-four.bit"
            output_path = root / "out.s16le"
            input_path.write_bytes(first_packet_records(ROOT / "tests/vectors/rfc8251/testvector01.bit", 34))
            ...
            self.assertEqual(completed.returncode, 0, completed.stderr)
            self.assertEqual(len(output_path.read_bytes()), 27520)
